...
export async function mountCouponCalendar(container, bridge) {
  ...
  const openMarkdown = typeof bridge.openMarkdown === "function" ? bridge.openMarkdown : () => undefined;
...
function renderUnscheduledCards() {
  ...
  el.unscheduledCards.querySelectorAll("[data-open-card-id]").forEach((button) => {
    button.addEventListener("click", (event) => {
      event.stopPropagation();
      const card = getCard(button.dataset.openCardId);
      if (card?.markdownPath) void openMarkdown(card.markdownPath);
    });
  ...
  return `
    <article class="mini-card ${card.id === state.selectedCardId ? "is-selected" : ""}"
      ...
      <div class="card-title-row">
        ${card.markdownPath
          ? `<button class="card-title card-title-link" type="button" data-open-card-id="${escapeAttribute(card.id)}" title="打开 Markdown">${escapeHtml(card.title || "未命名卡片")}</button>`
  ...
  replaceWithHtml(el.detailBody, `
    <form class="detail-form" id="detailForm">
      ...
      <div class="detail-actions">
        ${card.markdownPath ? `<button class="text-button" id="openMarkdownButton" type="button">打开 Markdown</button>` : ""}
...
function bindDetailForm(card, event) {
  ...
  root.querySelector("#openMarkdownButton")?.addEventListener("click", () => {
    if (card.markdownPath) void openMarkdown(card.markdownPath);
  });
